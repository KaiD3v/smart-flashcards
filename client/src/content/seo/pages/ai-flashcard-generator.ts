...
export const aiFlashcardGenerator: SeoPageContent = {
  ...
  sections: [
    section("problem", "The Manual Flashcard Problem", 2, [
      ...
      "MyRemynd closes the loop by sending generated cards straight into FSRS, so the time you save on typing goes directly into reviewing rather than managing exports and imports.",
    ]),
    section("how-it-works", "How AI Flashcard Generation Works", 2, [
      ...
      "MyRemynd integrates this pipeline into the platform. Generated cards are saved to your subject, assigned an initial FSRS state, and appear in your due queue according to the scheduling algorithm. There is no export step or format conversion.",
    ]),
    section("pdf-upload", "PDF Upload: The Fastest Path", 2, [
      ...
      "MyRemynd supports PDF upload alongside plain text input. Medical students upload pathophysiology packets. Law students upload case compendiums. Engineering students upload formula reference sheets. Each becomes a FSRS-scheduled deck within minutes.",
    ]),
    section("quality-control", "Editing and Quality Control", 2, [
      ...
      "MyRemynd provides inline editing for every card before and after generation. Quality control takes minutes compared to the hours manual creation would require. Once edited, cards enter FSRS scheduling and improve your retention immediately.",
    ]),
    section("fsrs-integration", "Why Generation Needs FSRS Scheduling", 2, [
      ...
      "MyRemynd connects generation to FSRS natively. Card creation and scheduling happen in one action. You never hold a CSV of generated cards wondering which app to import it into. The deck is live and scheduling from the first review.",
    ]),
    ...
    section("start-now", "Generate Your First Deck Today", 2, [
      "Pick the PDF you would least want to re-read before the exam. Upload it to MyRemynd, review the generated cards, and complete your first FSRS session. Most students finish the entire process in under thirty minutes.",
    ...
    section("batch-workflow", "Batch Generation for Heavy Course Loads", 2, [
      ...
      "MyRemynd organizes decks by subject so parallel courses stay separated. Monday anatomy upload, Tuesday pharmacology upload, daily reviews mixing due cards from both. FSRS handles the scheduling math while you maintain a single consistent review habit.",
  ...
  faqs: [
    {
      question: "What file formats does MyRemynd accept?",
      answer:
        "MyRemynd supports PDF uploads and pasted text for AI flashcard generation. Upload your lecture slides or reading assignments directly and start FSRS reviews the same day after a quick accuracy check on generated cards.",
    },
    {
    ...
    },
    {
      ...
      answer:
        "MyRemynd applies FSRS scheduling to every generated card automatically. Your deck is ready for spaced review immediately after generation with no extra setup or configuration required.",
    },
    {
    ...
    },
    {
      ...
      answer:
        "Paste transcribed text or typed summaries into MyRemynd for AI generation. Clear, text-based input produces the most accurate flashcard output before FSRS scheduling begins.",
    },
    {
